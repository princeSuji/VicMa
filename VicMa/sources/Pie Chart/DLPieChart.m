...
//        [self deselectAllSlices];
        _selectedSliceIndex = -1;
        [self notifyDelegateOfSelectionChangeFrom:selectedIndex to:_selectedSliceIndex];
    }
    else
    {
...
- (void)deselectAllSlices
{
    ...
    {
        [self setSliceDeselectedAtIndex:i];
    }
}
...
- (void)renderInLayer:(DLPieChart *)layerHostingView dataArray:(NSMutableArray*)dataArray legendsArray:(NSArray *)legends andColorsArray:(NSMutableArray *)colorsArray
{
    if(!self.DLDataArray)
...
//    }
    ...
    [self drawLegends:layerHostingView dataArray:[NSMutableArray arrayWithArray:legends]];
}
...
- (UIColor *)pieChart:(DLPieChart *)pieChart colorForSliceAtIndex:(NSUInteger)index
{
    ...
    return [self.DLColorsArray objectAtIndex:index];
}
